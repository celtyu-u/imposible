...
public class DatabaseService {
	...
	public DataGeneric deleteDetalleId(String table, String id) {
		DataGeneric dataGeneric = new DataGeneric();
		dataGeneric.setTable(table);

		try {
			int rows = repository.deleteDetalleRows(table,id);
			dataGeneric.setStatus(1);
		} catch (Exception e) {
			dataGeneric.setStatus(0);
			dataGeneric.setMessage(e.getMessage());
	...
	public DataGeneric getByDetalleId(String table, String id) {
		DataGeneric dataGeneric = new DataGeneric();
		dataGeneric.setTable(table);

		try {
			List<Map<String, Object>> rows = repository.getAllDetalleRows(table,id);
			List<List<Field>> datas = convertRowsToFields(rows, table);
	...
	public DataGeneric insert(String table, List<List<Field>> datas) {
		...
		try {
			...
			dataGeneric.setLastPk(repository.insertRows(table, rows));
			dataGeneric.setStatus(1);
			dataGeneric.setMessage("Insert successful");
		} catch (Exception e) {
			dataGeneric.setLastPk(-1);
			dataGeneric.setStatus(0);
			dataGeneric.setMessage(e.getMessage());
		}
	...
	private List<FieldDB> getDataLocal(String tableName) {
		...
		List<FieldDB> fieldsDB = TablesFields.stream().filter(field -> field.getNameTable().equalsIgnoreCase(tableName))
		...
		for (Map<String, Object> row : rows) {
			...
			row.forEach((key, value) -> {
				...
				Optional<FieldDB> fieldOptional = getDataLocal(tableName).stream()
						.filter(f -> f.getName().equalsIgnoreCase(key)).findFirst();

				fieldOptional.ifPresentOrElse(f -> {
	...
	private List<Map<String, Object>> convertFieldsToRows(List<List<Field>> datas, String tableName) {
		...
		for (List<Field> fields : datas) {
			...
			for (Field field : fields) {
				...
				Optional<FieldDB> result = fieldsDB.stream().filter(f -> f.getName().equals(nameField)).findFirst();
