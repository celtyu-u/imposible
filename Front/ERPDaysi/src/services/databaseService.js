...
export const databaseService = {
  ...
  delete: async (tables, id) => {
    try {
      const response = await apiClient.delete(
        `database/delete?table=${tables}&id=${id}`
      );
      return response.data;
    } catch (error) {
  ...
  getByDetalleId: async (tables, id) => {
    try {
      const response = await apiClient.get(
        `database/getByDetalleId?table=${tables}&id=${id}`
      );
      return response.data;
    } catch (error) {
      console.error("Error delete:", table, error);
  ...
  deleteDetalleId: async (tables, id) => {
    try {
      const response = await apiClient.delete(
        `database/deleteDetalleId?table=${tables}&id=${id}`
      );
      return response.data;
    } catch (error) {
      console.error("Error delete:", table, error);
